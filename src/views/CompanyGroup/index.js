...
import CompanyGroupServices from "../../services/companyGroupServices";
...
const CompanyGroup = memo((props) => {
        ...
        const getList = async () => {
            try {
                // setLoading(true);
                const response = await CompanyGroupServices.getAllItems();
                ...
                render: (text, record, _, action) => [
                    ...
                    <Popconfirm
                        ...
                        onConfirm={async () => {
                            ...
                            // @ts-ignore
                            await CompanyGroupServices.deleteItem(record.id)
        ...
        const searchSubmit = async () => {
            ...
            const data = defaultList.filter(item => {
                return (item.groupCode?.toLowerCase().includes(value.Code?.toLowerCase() || '')
                    && item.groupName?.toLowerCase().includes(value.Name?.toLowerCase() || ''))
            })
            setList(data);
        }
                        ...
                        editable={{
                            ...
                            onSave: async (rowKey, data, row) => {
                                ...
                                if (list.filter(item => item.id === rowKey).length > 0) {
                                    await CompanyGroupServices.updateItem(rowKey.toString() || '', data);
                                } else {
                                    await CompanyGroupServices.createItem(data);
                            ...
                            onDelete: async (rowKeys) => {
                                ...
                                await CompanyGroupServices.deleteItem(rowKeys);
                                message.success('Thành công');
                            },
...
CompanyGroup.displayName = "CompanyGroup"
